...
import { Card, CardBody, CardHeader, Button, Chip, Progress, Input, Alert } from "@heroui/react";
import { Activity, Brain, Zap, Target, TrendingUp, Eye, Shuffle, Link, Server } from 'lucide-react';
...
export class ColabAPIClient {
  ...
  async testConnection(maxRetries: number = 3): Promise<{ success: boolean; error?: string; details?: string }> {
    if (!this.baseUrl) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(`Connection attempt ${attempt}/${maxRetries} to ${this.baseUrl}`);
        ...
        if (!response.ok) {
          ...
          if (response.status === 502 || response.status === 503 || response.status === 504) {
            ...
            return { 
              ...
              details: 'Try waiting a moment and reconnecting, or restart the Colab cell.'
            };
          }
        ...
        if (responseText.trim().startsWith('<!DOCTYPE') || responseText.trim().startsWith('<html')) {
          ...
          if (responseText.includes('ngrok') && (responseText.includes('Visit Site') || responseText.includes('only for legitimate traffic'))) {
            return { 
              success: false, 
              error: 'Ngrok "Visit Site" page detected',
              details: 'You need to visit the ngrok URL in your browser first and click "Visit Site" to authorize access. This is ngrok\'s security feature.'
            };
          }
          
          if (attempt < maxRetries) {
            console.log('Received HTML instead of JSON, Flask server may be starting up, retrying...');
          ...
          return { 
            ...
            details: 'Flask server is not running properly. Check Colab notebook and restart the Flask server cell.'
          };
        }
        ...
        if (!this.isConnected) {
          return { 
            ...
            details: 'The Flask server is running but the neural network model is not loaded properly.'
          };
        }
        ...
        if (error instanceof Error && error.name === 'AbortError') {
          ...
          return { 
            ...
            details: 'The server is taking too long to respond. Check if the Colab notebook is still running.'
          };
        }
        ...
        if (error instanceof TypeError && error.message.includes('fetch')) {
          ...
          return { 
            ...
            details: 'Check if the ngrok URL is correct and the Colab notebook is running.'
          };
        }
  ...
  async predict(name: string, retryCount: number = 2): Promise<ColabInferenceStats | null> {
    ...
    for (let attempt = 1; attempt <= retryCount; attempt++) {
      try {
        console.log(`Prediction attempt ${attempt}/${retryCount} for name: ${name}`);
        ...
        if (response.ok) {
          ...
          if (attempt < retryCount && (response.status >= 500 || response.status === 502)) {
            // Server errors - retry
...
const ColabInferenceEngine: React.FC<ColabInferenceEngineProps> = ({
    ...
    try {
      ...
      if (result.success) {
        ...
        if (testResult) {
          ...
          setError('⚠️ Connected but prediction test failed. The server may be overloaded.');
        }
      } else {
        ...
        } else if (result.error?.includes('HTML') || result.error?.includes('JSON')) {
        ...
        } else if (result.error?.includes('502') || result.error?.includes('503') || result.error?.includes('504')) {
        ...
        } else if (result.error?.includes('timeout') || result.error?.includes('Network error')) {
  ...
  }, [isConnected, connectionHealth, lastSuccessfulCall]);  const tryRandomName = () => {
            ...
            <Alert
              ...
              description={
                <div className="space-y-2 text-sm">
                  <p>1. Open Google Colab and run the gender classification notebook</p>
            ...
            <Alert
              ...
              description={
                <div className="space-y-2 text-sm">
                  ...
                  <p>2. Click "Visit Site" on the ngrok security page</p>
                  <p>3. This authorizes the connection - then return here and connect</p>
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const text = await response.text();
                      setError(`Debug - Server responded with:\nStatus: ${response.status}\nContent-Type: ${response.headers.get('content-type')}\nFirst 300 chars: ${text.substring(0, 300)}...`);
                    } catch (err) {
                      setError(`Debug - Failed to reach server: ${err instanceof Error ? err.message : 'Unknown error'}`);
                    }
                  }}
                >
                  Debug
    ...
    return (
      <Card className="w-full">
        <CardBody className="text-center p-6">
          ...
          <p className="text-default-500 mb-4">Getting predictions from real neural network...</p>
          <Button
            variant="light"
            size="sm"
            onClick={disconnect}
    ...
    return (
      <Card className="w-full">
        <CardBody className="text-center p-6">
          ...
          <p className="text-default-500 mb-4">Start inference to see real neural network predictions</p>
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Server className={`w-4 h-4 ${
                connectionHealth === 'healthy' ? 'text-success' :
                connectionHealth === 'degraded' ? 'text-warning' : 'text-danger'
              }`} />
        ...
        <Card>
          ...
          <CardBody className="pt-2">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-small text-default-500 mb-1">Training Accuracy</p>
                <div className="flex items-center gap-2">
                  <Progress 
              ...
              <div>
                <p className="text-small text-default-500 mb-1">Validation Accuracy</p>
                <div className="flex items-center gap-2">
                  <Progress 
              ...
              <div>
                <p className="text-small text-default-500 mb-1">Training Loss</p>
                <div className="flex items-center gap-2">
                  <Progress 
              ...
              <div>
                <p className="text-small text-default-500 mb-1">Validation Loss</p>
                <div className="flex items-center gap-2">
                  <Progress 
      ...
      <Card>
        ...
        <CardBody className="pt-2">
          <div className="space-y-3">
            {Object.entries(stats.layer_activations).map(([layerName, intensity]) => (
