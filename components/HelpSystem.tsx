...
const HelpSystem: React.FC<HelpSystemProps> = ({ currentStep, onStepChange }) => {
  ...
  const quickStartSteps = [
    {
      ...
      description: "Start by dragging neural network layers from the sidebar to the canvas.",
      icon: "lucide:brain",
      action: "Drag an Input Layer to begin"
    },
    {
      title: "Connect Layers",
      description: "Click and drag from output ports to input ports to connect layers.",
      icon: "lucide:link",
    ...
    },
    {
      ...
      description: "Every network needs an output layer to produce predictions.",
      icon: "lucide:arrow-right",
      action: "Add an Output Layer and connect it"
    },
    {
      title: "Generate Code",
      description: "Click the code button to generate TensorFlow or PyTorch code.",
      icon: "lucide:code",
  ...
  const keyboardShortcuts = [
    { key: ["Ctrl", "S"], description: "Save current project", category: "Project" },
    ...
    { key: ["Ctrl", "C"], description: "Copy selected nodes", category: "Edit" },
    { key: ["Ctrl", "V"], description: "Paste nodes", category: "Edit" },
    { key: ["Space"], description: "Pan canvas (hold and drag)", category: "Navigation" },
    ...
    { key: ["Escape"], description: "Cancel current operation", category: "General" },
    { key: ["Ctrl", "L"], description: "Auto-layout network", category: "Layout" },
    { key: ["Ctrl", "G"], description: "Generate code", category: "Code" },
    { key: ["Ctrl", "E"], description: "Export project", category: "Export" },
  ];

  const mouseControls = [
    { action: "Left Click", description: "Select node or edge", icon: "lucide:mouse-pointer" },
    { action: "Right Click", description: "Context menu (planned)", icon: "lucide:menu" },
    { action: "Drag Node", description: "Move node around canvas", icon: "lucide:move" },
    { action: "Drag from Port", description: "Create connection between layers", icon: "lucide:link" },
    { action: "Scroll Wheel", description: "Zoom in/out of canvas", icon: "lucide:zoom-in" },
    { action: "Middle Click + Drag", description: "Pan around canvas", icon: "lucide:hand" },
    { action: "Double Click", description: "Fit view to all nodes", icon: "lucide:maximize" },
  ];

  const tips = [
    {
      title: "Start Simple",
      description: "Begin with basic architectures like Input → Dense → Output before building complex networks.",
      icon: "lucide:lightbulb",
      type: "beginner"
    },
    {
      title: "Use Templates",
      description: "Load pre-built templates for common architectures like CNNs, LSTMs, and Autoencoders.",
      icon: "lucide:template",
      type: "efficiency"
    },
    {
      title: "Validate Your Model",
      description: "Use the Model Validator to check for common architecture issues and best practices.",
      icon: "lucide:check-circle",
      type: "quality"
    },
    {
      title: "Monitor Performance",
      description: "Check the Performance Analysis to understand your model's computational requirements.",
      icon: "lucide:bar-chart",
      type: "optimization"
    },
    {
      title: "Save Your Work",
      description: "Use the Project Manager to save, organize, and share your neural network designs.",
      icon: "lucide:save",
      type: "workflow"
    },
    {
      title: "Experiment with Layers",
      description: "Try different layer types and configurations to see how they affect your network.",
      icon: "lucide:experiment",
      type: "learning"
    },
    {
      title: "Use Auto-Layout",
      description: "Click the layout button to automatically organize your network for better visualization.",
      icon: "lucide:layout",
      type: "organization"
    },
    {
      title: "Export to Colab",
      description: "Generate Jupyter notebooks that run directly in Google Colab for easy training.",
  ...
  const renderQuickStart = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-default-500">Follow these steps to create your first neural network</p>
      </div>
      ...
      <div className="space-y-4">
        {quickStartSteps.map((step, index) => (
          <Card 
            key={index} 
            className={(currentStep ?? -1) === index ? "border-primary bg-primary-50" : ""}
          >
            <CardBody className="p-4">
              <div className="flex items-start gap-4">
                <div className="flex-shrink-0">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                    (currentStep ?? -1) === index ? "bg-primary text-white" :
                    (currentStep ?? -1) > index ? "bg-success text-white" : "bg-default-200"
                  }`}>
                    {(currentStep ?? -1) > index ? (
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <Icon icon={step.icon} className="w-5 h-5 text-primary" />
                    <h4 className="font-semibold">{step.title}</h4>
                  </div>
                  <p className="text-sm text-default-600 mb-3">{step.description}</p>
  ...
  const renderShortcuts = () => (
    <div className="space-y-6">
      {["Project", "Edit", "Navigation", "General", "Layout", "Code", "Export"].map(category => {
    ...
    <div className="space-y-4">
      {mouseControls.map((control, index) => (
        <div key={index} className="flex items-center gap-4 p-3 bg-default-50 rounded-lg">
          <Icon icon={control.icon} className="w-6 h-6 text-primary" />
          <div className="flex-1">
              ...
              <div className="flex-1">
                ...
                <div className="flex gap-2">
                  ...
                  <Chip 
                    size="sm" 
                    variant="flat" 
                    color={resource.difficulty === "beginner" ? "success" : 
          ...
          {(onClose) => (
            <>
              ...
              <ModalFooter>
                ...
                <Button
                  color="primary"
                  onPress={() => window.open("https://github.com/mahendra189/neod", "_blank")}
