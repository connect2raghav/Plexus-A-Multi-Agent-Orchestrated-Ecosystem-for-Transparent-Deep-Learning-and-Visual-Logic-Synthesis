...
const ModelValidator: React.FC<ModelValidatorProps> = ({
  ...
  const analyzeModel = async () => {
    ...
    const inputNodes = nodes.filter(
      (n) => n.type === "inputLayer" || n.type === "textInput",
    );
    ...
    if (inputNodes.length === 0) {
      issues.push({
        ...
        description:
          "Your model needs at least one input layer to receive data.",
        suggestion:
          "Add an Input Layer from the Input/Output section in the sidebar.",
        severity: 10,
      });
    }
    ...
    if (inputNodes.length > 1) {
      issues.push({
        ...
        description:
          "Multiple input layers detected. This creates a multi-input model architecture.",
        suggestion:
          "Consider if you really need multiple inputs or merge them using concatenation.",
        severity: 4,
      });
    }
    ...
    if (outputNodes.length === 0) {
      issues.push({
        ...
        suggestion:
          "Add an Output Layer from the Input/Output section in the sidebar.",
        severity: 10,
      });
    }
    ...
    if (outputNodes.length > 1) {
      issues.push({
        ...
        description:
          "Multiple output layers detected. This creates a multi-output model.",
    ...
    const disconnectedNodes = nodes.filter(
      (node) => !connectedNodeIds.has(node.id) && node.type !== "textInput",
    );
    ...
    if (denseNodes.length > 10) {
      issues.push({
        ...
        description:
          "Networks with many dense layers can suffer from vanishing gradients.",
        suggestion:
          "Consider using residual connections, batch normalization, or reducing depth.",
        severity: 5,
      });
    }
    ...
    if (denseNodes.length > 2 && dropoutNodes.length === 0) {
      issues.push({
        ...
        description:
          "Deep networks without regularization are prone to overfitting.",
        suggestion:
          "Add Dropout layers or Batch Normalization between dense layers.",
        severity: 7,
      });
    }
    ...
    if (outputNode && outputNode.data.params) {
      ...
      if (units === 1 && activation !== "sigmoid") {
        issues.push({
          ...
          description:
            "For binary classification (1 output unit), sigmoid activation is recommended.",
          suggestion:
            "Change output activation to 'sigmoid' for binary classification.",
      ...
      if (units > 2 && activation !== "softmax") {
        issues.push({
          ...
          description:
            "For multi-class classification, softmax activation is recommended.",
          suggestion:
            "Change output activation to 'softmax' for multi-class classification.",
          nodeId: outputNode.id,
          severity: 6,
        });
      }
    ...
    if (convNodes.length > 0) {
      ...
      if (flattenNodes.length === 0) {
        issues.push({
          ...
          description:
            "Convolutional layers need a Flatten layer before dense layers.",
          suggestion:
            "Add a Flatten layer between convolutional and dense layers.",
          severity: 9,
        });
      }
      ...
      if (!hasPooling && convNodes.length > 1) {
        issues.push({
          ...
          description:
            "CNNs typically benefit from pooling layers to reduce spatial dimensions.",
    ...
    if (totalParameters > 1000000) {
      issues.push({
        ...
        suggestion:
          "Consider reducing layer sizes or using techniques like pruning.",
        severity: 5,
      });
    }
    ...
    if (dropoutNodes.length > 0) {
      recommendations.push(
        "👍 Good use of regularization with dropout layers.",
      );
    }

    if (nodes.some((n) => n.type === "batchnorm")) {
      recommendations.push(
        "👍 Batch normalization will help with training stability.",
      );
    }
    ...
    if (issues.length === 0) {
      recommendations.push(
        "🎉 No issues found! Your architecture follows best practices.",
      );
    }
                      ...
                      <CardBody className="pt-0">
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <p className="text-lg">
                              {analysis.totalParameters.toLocaleString()}
                            </p>
                          </div>
                          <div>
                            <p className="text-sm font-medium text-default-700">
                          ...
                          <div>
                            ...
                            <Chip
                              color={
                                analysis.inferenceSpeed === "fast"
                                  ? "success"
                                  : analysis.inferenceSpeed === "medium"
                                    ? "warning"
                                    : "danger"
                              }
                              size="sm"
