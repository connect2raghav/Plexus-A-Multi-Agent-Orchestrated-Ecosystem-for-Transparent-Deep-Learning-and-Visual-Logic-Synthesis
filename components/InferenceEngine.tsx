...
import { Activity, Brain, TrendingUp, Eye, Shuffle } from "lucide-react";
...
export class GenderClassificationInference {
  ...
  public predict(name: string): InferenceStats {
    ...
    this.topology.forEach((nodeId) => {
      ...
      switch (node.type) {
        ...
        case "dense":
          ...
          if (this.weights[nodeId]) {
            for (
              let i = 0;
              i < denseUnits && i < this.weights[nodeId].length;
              i++
            ) {
              let sum = 0;
              ...
              for (
                let j = 0;
                j < currentOutput.length && j < this.weights[nodeId][i].length;
                j++
              ) {
      ...
      layerOutputs[nodeId] =
        currentOutput.reduce((sum, val) => sum + Math.abs(val), 0) /
...
const InferenceEngine: React.FC<InferenceEngineProps> = ({
  ...
  useEffect(() => {
    inferenceEngineRef.current = new GenderClassificationInference(
      nodes,
      edges,
    );
  }, [nodes, edges]);
      ...
      <Card className="w-full">
        <CardBody className="text-center p-6">
          ...
          <p className="text-default-500 mb-4">
            Start inference to see real-time stats
          </p>
          <Button
            color="primary"
        ...
        <CardBody className="pt-2">
          <div className="space-y-3">
            {Object.entries(stats.layerOutputs).map(([layerId, intensity]) => {
              ...
              if (!node || !node.type || ["textInput"].includes(node.type))
                return null;
