...
# Load .env from backend/ directory (works both from cli.py and FastAPI)
...
def _auto_detect_provider() -> LLMProvider:
    ...
    raise EnvironmentError(
        ...
        "  • For Gemini  (free tier): set GEMINI_API_KEY  in backend/.env\n"
        "  • For OpenAI             : set OPENAI_API_KEY  in backend/.env\n"
                ...
                if self._is_auth_error(exc):
                    ...
                    if "insufficient_quota" in msg or "billing" in msg:
                        raise RuntimeError(
                            ...
                            "  • Or switch to Gemini (free tier): add GEMINI_API_KEY to backend/.env\n"
                    ...
                    raise RuntimeError(
                        ...
                        "  • Or switch to Gemini: add GEMINI_API_KEY to backend/.env"
                    ) from exc
