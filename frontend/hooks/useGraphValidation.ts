...
export function useGraphValidation(
  ...
  return useMemo(() => {
    ...
    const datasetNodes = nodes.filter((n) => n.type === "dataset" && n.data?.columns?.length > 0);
    const inputNodesForShape = nodes.filter((n) => n.type === "inputLayer" && n.data?.params?.shape);
    if (datasetNodes.length > 0 && inputNodesForShape.length > 0) {
      ...
      if (Array.isArray(inputShape) && inputShape.length === 1 && inputShape[0] !== dsColCount) {
        issues.push(
          makeIssue(
